...
cl::OptionCategory CallgraphCategory(
	"Callgraph Options");

cl::opt<string> optOutFilename(
  "o", cl::desc("Specify output CSV filename"), 
  cl::value_desc("filename"), cl::init("callgraph.csv"), cl::cat(CallgraphCategory));

cl::opt<AnalysisType> optAnalysisType(
	cl::desc("Resolve indirect call targets with:"),
	cl::values(
		clEnumVal(mlta_pref, "Prefer MLTA, fallback to TA if MLTA failed (default)"),
		clEnumVal(mlta_only, "Find targets of indirect calls based on MLTA"),
		clEnumVal(ta_only, "Find targets of indirect calls based on type analysis (TA)")
	),
